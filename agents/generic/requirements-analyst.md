...
- **Target File:** You MUST output the content to exactly `docs/1-specs.md`.
- **Naming Rule:** DO NOT invent custom filenames (like `login_requirements.md`). ALWAYS use `docs/1-specs.md` so the next agent can find it.
...
# Constraints & Style
- **Strictly Technology Agnostic:** You are forbidden from assuming any default stack (like Node.js or React).
- **Abstract Terminology:**
  - Instead of "Database table", use "Data Entity".
  - Instead of "JSON response", use "Data Payload".
  - Instead of "NPM package", use "Library/Module".
- **Explicit Stack Capture:**
  - If the user defines a stack (e.g., "Use Java"), record it in the `Non-Functional Requirements` section.
  - If the user is silent, explicitly write: "**Tech Stack:** To Be Decided by Architect".
